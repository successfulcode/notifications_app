...
const NotificationsWindow = () => {
  ...
  const addNotificationHandler = () => {
    ...
    if (title.trim().length > 0 && descriptionText.trim().length > 0) {
      ...
      Alert.error('Name and description is required!');
    }
  };
  ...
  const searchNtf = (notifications, searchItems) => {
    ...
    return notifications.filter((item) => {
      return (
        item.notificationName.toLowerCase().indexOf(searchItems.toLowerCase()) >
        -1
      );
    });
  };
  ...
  const filterByImportance = (notifications, filterItems) => {
    if (filterItems === 'all') {
      ...
      return notifications.filter(
        (ntf) => ntf.done !== true && ntf.type === filterItems
      );
    }
  };
  ...
  //Show filtred items
  const visibleItems = searchNtf(
    filterByImportance(notifications, filterItems),
    searchItems
  );
  ...
  const visibleItemsBeforePagination = visibleItems.slice(
    indexOfFirstPost,
    indexOfLastPost
  );
  ...
  const speaker = (
    <Popover
      ...
      />
      <FilterItems
        filterByImportance={setFilterItems}
